...
public class RegisterService {
    ...
    public void sendEmail(String email, int type) {
        ...
        if (type == SendEmailEnum.REGISTER.getCode()) {
            simpleMailMessage.setSubject("注册ProgramChat账号");
            simpleMailMessage.setText("欢迎加入ProgramChat！ 您的验证码是：" + code + "，请在5分钟内完成注册。");
        } else if(type == SendEmailEnum.UPDATE_PWD.getCode()) {
            simpleMailMessage.setSubject("ProgramChat密码找回");
            simpleMailMessage.setText("欢迎加入ProgramChat！ 您的验证码是：" + code + "，请在5分钟内完成密码的修改。");
        }
